...
def node_loc(node: ast.AST) -> int:
    # Safe access to lineno/end_lineno (not always present for all
    # nodes/static analyzers)
...
def is_decorated(
    func: Union[ast.FunctionDef, ast.AsyncFunctionDef], name: str
) -> bool:
...
def analyze_class(py_path: Path, cls: ast.ClassDef) -> ClassMetrics:
    ...
    avg_method_loc = (
        round(sum(m.loc for m in methods) / total_methods, 2)
        if total_methods else 0.0
    )
    avg_complexity = (
        round(sum(m.complexity for m in methods) / total_methods, 2)
        if total_methods else 0.0
    )
...
def write_reports(metrics: List[ClassMetrics]) -> None:
    ...
    md_lines = [
        ...
        "",
        ("| Klasse | Datei | LOC | Methoden (pub/prv) | static | class | "
         "⌀ LOC/Method | ⌀/max Komplexität | Attrs (class/instance) | Doku |"),
        "|---|---|---:|---:|---:|---:|---:|---:|---|---|",
    ]
    for m in metrics:
        md_lines.append(
            f"| {m.class_name} | {m.file} | {m.loc} | {m.methods} "
            f"({m.public_methods}/{m.private_methods}) | "
            f"{m.static_methods} | {m.class_methods} | {m.avg_method_loc} | "
            f"{m.avg_complexity}/{m.max_complexity} | "
            f"{','.join(m.class_attributes) or '-'} / "
            f"{','.join(m.instance_attributes) or '-'} | "
...
def main() -> None:
    ...
    important = {
        "RuleBasedLinker", "LinkRule", "LinkMatch",
        "CrossVaultLinker", "CortexConfig", "CortexAIClient"
    }
